...
import CompetenceAreasAdd from './CompetenceAreasAdd';
import CompetenceAreasEdit from './CompetenceAreasEdit';
import CompetenceAreasReorder from './CompetenceAreasReorder';

export const CompetenceAreas: React.FC = () => {
  const { t } = useTranslation();
  const [isCompetenceFormOpen, setIsCompetenceFormOpen] = useState(false);
  const [isCompetenceEditFormOpen, setIsCompetenceEditFormOpen] = useState(false);
  const [isCompetenceReorderFormOpen, setIsCompetenceReorderFormOpen] = useState(false);
  ...
  return (
    <>
      {!isCompetenceFormOpen && !isCompetenceEditFormOpen && !isCompetenceReorderFormOpen && ( // Hide when any form is open
        <div className=" p-4 bg-white rounded shadow mt-5 text-base">
          <div className="flex justify-between items-center mb-4 ml-5">
            <h2 className="text-2xl font-bold mr-5 times-new-roman-font">{t('CompetenceAreas')}</h2>
            <ActionButtons
          ...
          <ul className="circle-list text-base">
            <li><i className="mb-3 fas fa-circle-notch circle-icon"></i> .Net</li>
            <li><i className="mb-3 fas fa-circle-notch circle-icon"></i> C+</li>
            <li><i className="mb-3 fas fa-circle-notch circle-icon"></i> React Native</li>
            <li><i className="mb-3 fas fa-circle-notch circle-icon"></i> Angular</li>
          </ul>
          <button
      ...
      {isCompetenceFormOpen && <CompetenceAreasAdd onClose={handleCloseCompetenceForm} />}
      {isCompetenceEditFormOpen && <CompetenceAreasEdit onClose={handleCloseCompetenceEditForm} />}
      {isCompetenceReorderFormOpen && <CompetenceAreasReorder onClose={handleCloseCompetenceReorderForm} />}
    </>
  );
};
