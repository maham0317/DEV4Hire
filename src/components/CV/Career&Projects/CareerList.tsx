...
export const CareerList: React.FC = () => {
  ...
  return (
    <>
      {!isCareerAddFormOpen && !isCareerEditFormOpen ? ( 
        <div className="p-4 rounded bg-white shadow mt-5 text-base">
          <div className="flex justify-between items-center mb-4 ml-5">
            <h2 className="text-2xl font-bold mr-5 times-new-roman-font">{t('Career&Projects')}</h2>
            <ActionButtons onAdd={handleCareerAddLinkClick} onEdit={handleCareerEditLinkClick} />
          </div>
      ...
      {isCareerEditFormOpen && <CareerEdit careerData={careerData} onClose={handleCloseCareerEditForm} />}
    </>
  );
};
