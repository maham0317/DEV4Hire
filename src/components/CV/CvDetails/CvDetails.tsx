  ...
  return (
    <>
      <div className="flex justify-between items-center mb-5 ml-5">
        <h2 className="text-2xl font-bold mr-5 times-new-roman-font">{t('ParseCv')}</h2>
      </div>

      <div className="flex ml-5 mt-2 mb-2">
        <p className="text-black-600 mr-5 text-base">{t('UploadInstruction')}</p>
      </div>
      ...
      <div className="flex ml-5 mt-5">
        {isEditingTitle ? (
          <EditTitleForm onSave={handleSaveTitle} onCancel={handleCancelEditTitle} />
        ) : (
          <>
            <h1 className="text-3xl font-bold ml-3 mt-5 times-new-roman-font">{title}</h1>
            <a href="#edit-title-link" className="text-blue-700 ml-5 mt-5 font-semibold" onClick={handleEditTitleClick}>
              {t('EditTitleLink')}
            </a>
          </>
        )}
      ...
      <h2 className="text-2xl font-bold ml-3 mt-5 times-new-roman-font">{t('ProfileIntro')}</h2>
      <div className="flex items-center ml-3 mt-5">
        ...
        <div className="flex ml-auto">
          {isEditingName ? (
            <EditNameForm onSave={handleSaveName} onCancel={handleCancelEditName} />
          ) : (
            <>
              <p>{name}</p>
              <a href="#edit-name" className="text-blue-700 font-semibold ml-auto" onClick={handleEditNameClick}>
                {t('EditLink')}
              </a>
            </>
          )}
      ...
      <div className="flex items-center ml-3 mt-5">
        <p className="font-bold">
          {t('AvailableFrom')} <i className="fas fa-question-circle text-gray-300"></i>
        </p>
      ...
      <p className="text-black-600 ml-2 text-sm font-semibold">Jeg har mer enn 18 års erfaring og jobber som Fullstack systemutvikler med hovedfokus på utvikling ...</p>
      <a href="#edit-link" className="text-blue-700 ml-auto font-semibold">
        Read more
      </a>
      <p className="text-xl font-bold mt-5 times-new-roman-font">{t('CVStrength')}</p>
