...
const IndustryTypeListing: FC = (): JSX.Element=> {
    ...
    const { isLoading, data, formData, filters, addNewIndustry, handleEdit, handleDelete, handleClose, onSuccess, isOpen, isEdit, isConfirm, onCloseConfirm, onConfirmSuccess, onPageChange, setFilters } = useIndustryTypeListing();
    ...
    const columns: ColumnProps<IndustryTypeModel>[] = [
        ...
        },
        {
            ...
            render: (_, record)=> <span className='cursor-pointer' onClick={()=> handleEdit(record)}>{record.IndustryName}</span>
        },
        {
            key: 'Description',
    ...
    return (
        <div className='bg-blue-50 p-4 h-screen'>
            <div className='flex flex-col gap-3 p-3'>
                <p className='text-xl text-indigo-900 bg-blue-50 font-montserrat font-normal'>{t('IndustryTypeListing.Title')}</p>
                <Button size='md' color='primary' className='w-40 h-10 rounded' onClick={addNewIndustry}><FaPlus className='mt-0.5 mr-2 h-4 w-4' />{t('IndustryTypeListing.Button.CreateNew')}</Button>
            </div>
            <Card className='border-1px rounded-none'>
                <div className='flex flex-row justify-between align-item-center p-2'>
                    <p className='text-xl text-indigo-900 font-semibold'>{t('IndustryTypeListing.Table.Title')}</p>
                    <TextInput style={{width: 312}} placeholder={t('IndustryTypeListing.Input.Search.Placeholder')} onChange={e=> setFilters(pre=> ({...pre, 'SearchTerm': e.target.value, CurrentPage: 1}))} addon={<FaSearch />} />
                </div>
            ...
            <ConfirmationModal messageString={t('IndustryTypeListing.Modal.Confirmation.Title')} isOpen={isConfirm? true: false} onClose={onCloseConfirm} onSuccess={onConfirmSuccess} />
        </div>
    );
};
